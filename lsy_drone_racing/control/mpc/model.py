...
class DeltaModel:
    ...
    def setup_symbolics(self):
        ...
        f3 = cs.MX.sym('f3')
        f4 = cs.MX.sym('f4')
        thrusts = cs.vertcat(f1, f2, f3, f4)

        # Define state variables.
        X = cs.vertcat(pos, pos_dot, ang, rate, thrusts)
        ...
        Mb = cs.vertcat(length / cs.sqrt(2.0) * (f1 + f2 - f3 - f4),
                        ...
                        gamma * (f1 - f2 + f3 - f4))
        rate_dot = Jinv @ (Mb - (cs.skew(rate) @ J @ rate))
        ang_dot = cs.blockcat([[1, cs.sin(phi) * cs.tan(theta), cs.cos(phi) * cs.tan(theta)],
                                [0, cs.cos(phi), -cs.sin(phi)],
                                [0, cs.sin(phi) / cs.cos(theta), cs.cos(phi) / cs.cos(theta)]]) @ rate
        X_dot = cs.vertcat(pos_dot, pos_ddot, ang_dot, rate_dot, U)

        Y = cs.vertcat(pos, pos_dot, pos_ddot, ang, rate, ang_dot, thrusts)
